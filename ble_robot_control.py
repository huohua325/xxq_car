...
class LidarPointCloud:
    """激光雷达点云数据（360点CSV格式）"""
    def __init__(self, timestamp: int, points: list):
        """
        Args:
            timestamp: 时间戳（毫秒）
            points: 点云列表 [{'angle': 0, 'distance': 1.2}, ...]
        """
        self.timestamp = timestamp
        self.points = points  # [{'angle': int, 'distance': float}, ...]
...
class SimpleBLERobotComm:
    ...
    def connect(self) -> bool:
        """连接BLE设备"""
        if self.verbose:
            print(f"[INFO] 连接BLE设备: {self.address}")
    ...
    def _parse_csv(self, line: str):
        ...
        try:
            if data_type == 'MPU' and len(parts) >= 10:
                ...
                # 🆕 新格式：CSV点云数据
                # 格式: LIDAR,timestamp,num_points,angle1,dist1,angle2,dist2,...
                if len(parts) >= 3:
                    timestamp = int(parts[1])
                    ...
                    self.latest_lidar = LidarPointCloud(timestamp, points)
                    self.stats['lidar_count'] += 1
                    if self.on_lidar_update:
                        self.on_lidar_update(self.latest_lidar)
